...
const portfolioSlice = createSlice({
    ...
    reducers: {
        addToPortfolio: (
            state,
            action: PayloadAction<Omit<PortfolioItem, 'type'>>
        ) => {
            const exists = state.items.some(
                (item) =>
                    item.id === action.payload.id && item.type === 'portfolio'
            );
            if (!exists) {
        ...
        addToWatchlist: (
            state,
            action: PayloadAction<Omit<WatchlistItem, 'type'>>
        ) => {
            const exists = state.items.some(
                (item) =>
                    item.id === action.payload.id && item.type === 'watchlist'
            );
            if (!exists) {
